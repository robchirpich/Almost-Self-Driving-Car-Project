...
while(1):
    if curFrame < NFRAMES:
        ...
        deg = rad2deg(rad)
        ser1.write(struct.pack("<f", deg))
        pred_end = time.time()

        # DISPLAY THE VIDEO WITH THE STEERING ANGLE
        cv2.putText(img2, str(deg), (0, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.imshow('Video with Angle', img2)
        cv2.waitKey(1)
